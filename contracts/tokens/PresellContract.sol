...
contract PresellContract { 
    ...
    event InvestmentToPresell(address indexed investor, uint256 amount);
    ...
    function invest(uint256 _amount) InvestorOnly(msg.sender) external {
        ...
        if (investorBalance[msg.sender] == 0) {
            IPresellNFT(nftAddress).awardItem(msg.sender, nftURI);
        }
        ...
        emit InvestmentToPresell(msg.sender, _amount);
    }
